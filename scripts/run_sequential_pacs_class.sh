#!/usr/bin/env bash
# Sequential class removal on PACS (class-wise counterpart of CoUn's Fig. 6):
# 5 stages, one class each (0 dog -> 1 elephant -> 2 giraffe -> 3 guitar -> 4 horse).
# Runs ModULE, SalUn and Boundary Shrink, each chained over the stages in
# config/sequential/pacs_*_seq_class.yaml, plus the per-stage Retrain reference
...
#   bash scripts/run_sequential_pacs_class.sh
set -uo pipefail
