...
$PY learn.py --config config/learn/moe_baseline/moe_baseline_M12_k4.yaml > "$L/moe_baseline_learn.log" 2>&1
st=$?; echo "[exit $st]"
...
for scen in class domain; do
  ...
  $PY unlearn.py --config "config/learn/moe_baseline/unl_moe_${scen}.yaml" > "$L/MoE__${scen}.log" 2>&1
